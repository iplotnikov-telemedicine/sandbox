...
view: offices {
  ...
  measure: sweede_offices_count {
    type: count
    filters: [is_sweede: "yes"]
  }
  ...
  dimension: sweede_is_accept_order {
    type: yesno
    sql: ${TABLE}.potify_is_accept_order ;;
  }

  measure: sweede_accept_offices_count {
    type: count
    filters: [sweede_is_accept_order: "yes"]
  }
  ...
  dimension: selling_type {
    ...
    case: {
      when: {
        sql: ${TABLE}.selling_type = 'office' ;;
        label: "storefront office"
      }
      when: {
        sql: ${TABLE}.selling_type = 'ondemand_office' ;;
        label: "on-demand office"
      }
      when: {
        sql: ${TABLE}.selling_type = 'ondemand_storage' ;;
        label: "on-demand storage"
      }
      when: {
        sql: ${TABLE}.selling_type = 'storage' ;;
        label: "ordinary storage"
      }
    }
  }

  measure: storefront_offices_count {
    type:  count
    filters: [selling_type: "storefront office"]
  }

  measure: ondemand_offices_count {
    type:  count
    filters: [selling_type: "on-demand office"]
  }
